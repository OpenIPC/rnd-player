/**
 * Web Worker for extracting per-macroblock QP values from H.264 fMP4 segments.
 *
 * Pipeline:
 *   1. mp4box.js extracts H.264 NAL units from the fMP4 media segment
 *   2. Finds the target frame (closest to targetTime)
 *   3. Builds Annex B buffer (SPS+PPS+slices with start codes) and feeds
 *      to JM reference decoder WASM in one call
...
/**
 * Cached decoder instance — reused across decode requests to avoid
 * repeatedly allocating a new 64MB WASM instance.
 * Nulled after WasiExit (decoder destroyed); recreated on next request.
 */
let cachedDecoder: Jm264QpInstance | null = null;

async function getDecoder(): Promise<Jm264QpInstance> {
  if (cachedDecoder) return cachedDecoder;
  cachedDecoder = await createJm264QpDecoder();
...
async function handleDecode(msg: QpMapWorkerRequest & { type: "decode" }) {
  const { initSegment, mediaSegment, targetTime } = msg;
  console.log("[QP worker] handleDecode: targetTime=%f, init=%d bytes, media=%d bytes",
    targetTime, initSegment.byteLength, mediaSegment.byteLength);

  // 1. Extract parameter sets from init segment
  const paramSets = extractParameterSets(initSegment);
  ...
  // 6. Get (or create) JM264 decoder and decode
  let decoder;
  try {
    decoder = await getDecoder();
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[QP worker] decoder creation failed:", msg);
    const response: QpMapWorkerResponse = { type: "error", message: `Decoder init failed: ${msg}` };
    self.postMessage(response);
    return;
  }

  console.log("[QP worker] calling decodeFrame (%d bytes)...", annexB.length);
  const t0 = performance.now();
  const frameReady = decoder.decodeFrame(annexB);
  console.log("[QP worker] decodeFrame → %s, widthMbs=%d, heightMbs=%d (%.0fms)",
    frameReady, decoder.getWidthMbs(), decoder.getHeightMbs(), performance.now() - t0);

  // Flush to get any remaining frames (handles B-frame reorder)
  let hasFrame = frameReady;
  if (!hasFrame) {
    const flushed = decoder.flush();
    console.log("[QP worker] flush → %s", flushed);
    if (flushed) hasFrame = true;
  }

  // If decoder was destroyed by WasiExit during decode, discard cache
  ...
  // 7. Read QP map
  const { qpValues, count } = decoder.copyQps();
  const widthMbs = decoder.getWidthMbs();
  const heightMbs = decoder.getHeightMbs();

...
- `e2e/hevc.spec.ts` — 6 tests. HEVC/H.265 DASH playback, frame stepping, filmstrip, and capability discovery (see HEVC Tests below).
- `e2e/av1.spec.ts` — 6 tests. AV1 DASH playback, frame stepping, filmstrip, and capability discovery (see AV1 Tests below).
...
4. **HEVC DASH fixture** — A 60-second HEVC/H.265 DASH stream with the same frame counter overlay and audio, packaged with 2 video renditions (1080p @ 2000k, 480p @ 400k). Generated by `e2e/generate-dash-fixture.sh` as a third step, gated on `libx265` availability in ffmpeg. Encoded with `-tag:v hvc1` (required for browser compatibility). Output in `$DASH_FIXTURE_DIR/hevc/`. `loadPlayerWithHevcDash(page)` serves files from memory via route interception. Gated on `isHevcDashFixtureAvailable()`. Only 2 renditions (vs 5 for H.264) because libx265 is ~5-10x slower.

5. **AV1 DASH fixture** — A 60-second AV1 DASH stream with the same frame counter overlay and audio, packaged with 2 video renditions (1080p @ 2000k, 480p @ 400k). Generated by `e2e/generate-dash-fixture.sh` as a fourth step, gated on AV1 encoder availability in ffmpeg. Prefers `libsvtav1` (fast, `-preset 12`) over `libaom-av1` (slow, `-cpu-used 8 -row-mt 1`). Output in `$DASH_FIXTURE_DIR/av1/`. `loadPlayerWithAv1Dash(page)` serves files from memory via route interception. Gated on `isAv1DashFixtureAvailable()`. Only 2 renditions (same as HEVC).

**Shared test utilities** (`e2e/helpers.ts`): Common functions used across multiple test files:
- `seekTo(page, time)` — seek + poll for `seeking=false` with retry loop and 3s per-attempt timeout
...
- `probeHevcMseSupport(page)` — checks `MediaSource.isTypeSupported` for hvc1/hev1
- `probeHevcWebCodecsSupport(page)` — checks `VideoDecoder.isConfigSupported` for hvc1
...
**HEVC/H.265 has limited CI support.** Unlike H.264, HEVC requires hardware decoders or platform-specific codec extensions that are generally unavailable on CI VMs. `MediaSource.isTypeSupported` is unreliable for HEVC — Firefox reports support on both macOS and Linux, but `player.load()` fails silently. The HEVC E2E tests use a two-stage skip strategy: first probe via `isTypeSupported`, then catch load failure in `tryLoadHevcDash()`. HEVC B-frame reordering (bframes=3 from libx265) causes ±3 frame seek offsets on platforms where playback works (e.g. WebKitGTK), so HEVC tests use frame tolerance instead of exact matching.

**WebKitGTK seeks stall under CI load.** On ubuntu-latest, WebKitGTK's media pipeline intermittently fails to complete seek operations — `video.seeking` stays `true` indefinitely, or `seeked` events never fire. This happens when setting `currentTime` (even to the current position) under VM resource pressure. All seek waits in E2E helpers have timeouts: `seekTo()` uses a 3s per-attempt deadline on `while (video.seeking)`, and `loadPlayerWith*()` functions use `Promise.race` with 5s on `await seeked`. Without these, stuck seeks cause 30s test timeouts that appear as random flakes.
...
2. `seekTo(page, time)` sets `currentTime` and polls until `seeking === false`, with a retry loop to handle Shaka Player's internal DASH seeks that can override the target position. Each attempt has a 3s deadline on the `while (video.seeking)` loop to prevent indefinite hangs on WebKitGTK
...
- **WebKitGTK seeks stall under CI VM load**: On ubuntu-latest, WebKitGTK's media pipeline occasionally fails to complete seek operations — `video.seeking` stays `true` indefinitely, or `seeked` events never fire. This is intermittent and load-dependent. Setting `video.currentTime = 0` when already at time 0 can trigger a stuck seek. The fix is deadlines on all unbounded seek waits: `seekTo()` has a 3s per-attempt timeout on its `while (video.seeking)` loop, `loadPlayerWith{Dash,EncryptedDash,HevcDash}()` use `Promise.race` with a 5s timeout on `await seeked`, and the filmstrip click seek in `ocr.spec.ts` has a 5s deadline. Without these timeouts, a single stuck seek causes a 30s test timeout with no useful diagnostic.
...
`e2e/hevc.spec.ts` — Tests HEVC/H.265 DASH playback and filmstrip thumbnail decoding. Uses the HEVC DASH fixture. Most tests skip on platforms without HEVC support.

```bash
DASH_FIXTURE_DIR=/tmp/dash-fixture npx playwright test e2e/hevc.spec.ts --project=chromium
```

**6 tests in 3 categories:**

| Category | Tests | What's verified |
|----------|-------|-----------------|
| Capability probe | Reports MSE + WebCodecs support | Always runs, logs results as Playwright annotations for CI discovery |
| Playback | t=0 → ~0000, t=5 → ~0150, ArrowRight → ~0001, play advances | Shaka Player HEVC MSE playback (±3 frame tolerance for B-frame offsets) |
| Filmstrip | Thumbnails render after loading | WebCodecs VideoDecoder HEVC → filmstrip thumbnail pipeline |

**Runtime skip strategy (two-stage):**
1. **API probe** — `probeHevcMseSupport(page)` checks `MediaSource.isTypeSupported('video/mp4; codecs="hvc1.1.6.L93.B0"')`. Skips immediately if false. This catches Chromium and Edge, which honestly report no HEVC MSE support.
2. **Load-failure catch** — `tryLoadHevcDash(page)` wraps `loadPlayerWithHevcDash` in try/catch. Firefox reports `isTypeSupported=true` but `player.load()` fails silently (controls never appear). The 30s load timeout is caught and the test skips with "HEVC MSE reported but player failed to load". Test timeout is set to 90s to accommodate this.

**HEVC B-frame tolerance**: The libx265 encoder produces IBBP GOP structure (bframes=3). On platforms where HEVC MSE works (e.g. WebKitGTK), composition time offsets from B-frame reordering can shift the displayed frame by ±3 relative to the exact frame number. HEVC seek tests use `toBeLessThanOrEqual(3)` tolerance, unlike H.264 tests which use exact matching.

**Fixture generation** requires `libx265` in ffmpeg. The `e2e/generate-dash-fixture.sh` script checks `ffmpeg -encoders | grep libx265` and skips if unavailable. Ubuntu/macOS/Windows CI ffmpeg packages all include libx265. Output in `$DASH_FIXTURE_DIR/hevc/`.

### AV1 Tests

`e2e/av1.spec.ts` — Tests AV1 DASH playback and filmstrip thumbnail decoding. Uses the AV1 DASH fixture. Tests skip on platforms without AV1 support.

```bash
DASH_FIXTURE_DIR=/tmp/dash-fixture npx playwright test e2e/av1.spec.ts --project=chromium
```
...
| Category | Tests | What's verified |
|----------|-------|-----------------|
| Capability probe | Reports AV1 MSE + WebCodecs support | Always runs, logs results as Playwright annotations for CI discovery |
| Playback | t=0 → ~0000, t=5 → ~0150, ArrowRight → ~0001, play advances | Shaka Player AV1 MSE playback (±3 frame tolerance) |
| Filmstrip | Thumbnails render after loading | WebCodecs VideoDecoder AV1 → filmstrip thumbnail pipeline |
...
**Encoder selection**: The fixture generation script prefers `libsvtav1` (SVT-AV1, ~10-50x faster than reference) with `-preset 12` (fastest). Falls back to `libaom-av1` (reference encoder) with `-cpu-used 8 -row-mt 1`. Skips if neither is available.

**Fixture generation** requires `libsvtav1` or `libaom-av1` in ffmpeg. The `e2e/generate-dash-fixture.sh` script checks both encoders and skips if unavailable. Ubuntu/macOS/Windows CI ffmpeg packages typically include at least `libaom-av1`. Output in `$DASH_FIXTURE_DIR/av1/`.

### Codec Support Matrix

Observed codec support across CI browser/OS combinations, based on E2E test results:

**H.264/AVC** (all fixtures except HEVC):

| Runner | Browser | H.264 MSE | H.264 WebCodecs | Notes |
|--------|---------|-----------|-----------------|-------|
| `ubuntu-latest` | chromium | Yes | Yes | I420 pixel format |
| `ubuntu-latest` | firefox | Yes | Yes | Requires `gstreamer1.0-libav`; BGRX format |
| `ubuntu-latest` | webkit | Yes | Yes | WebKitGTK; height rounded to 239px |
| `macos-latest` | webkit | Yes | Yes | NV12 pixel format |
| `macos-latest` | firefox | Yes | Yes | VideoToolbox; BGRX format |
| `windows-latest` | edge | Yes | Yes | I420 pixel format |

**HEVC/H.265** (`e2e/hevc.spec.ts`):

| Runner | Browser | HEVC MSE | HEVC WebCodecs | Notes |
|--------|---------|----------|----------------|-------|
| `ubuntu-latest` | chromium | No | No | Probe returns false; tests skip |
| `ubuntu-latest` | firefox | Probe: yes, actual: no | — | `isTypeSupported` lies; load fails; tests skip |
| `ubuntu-latest` | webkit | Partial | Partial | Loads but ±3 frame offset; filmstrip fails |
| `macos-latest` | webkit | Yes | Probe: yes, actual: no | Playback works; filmstrip thumbnails don't render |
| `macos-latest` | firefox | Probe: yes, actual: no | — | `isTypeSupported` lies; load fails; tests skip |
| `windows-latest` | edge | No | No | Probe returns false; tests skip |

**AV1** (`e2e/av1.spec.ts`):

| Runner | Browser | AV1 MSE | AV1 WebCodecs | Notes |
|--------|---------|---------|---------------|-------|
| `ubuntu-latest` | chromium | Yes | Yes | Full software decode; filmstrip works |
| `ubuntu-latest` | firefox | Yes | Yes | dav1d decoder; filmstrip works |
| `ubuntu-latest` | webkit | Yes | Probe: yes, actual: no | WebKitGTK; playback works; filmstrip thumbnails don't render |
| `macos-latest` | webkit | Probe: yes, actual: no | Probe: yes, actual: no | `isTypeSupported` lies; `readyState` stays at 1; all tests skip |
| `macos-latest` | firefox | Yes | Yes | dav1d decoder; filmstrip works |
| `windows-latest` | edge | Yes | Yes | Full software decode; filmstrip works |

**ClearKey DRM** (encrypted H.264, `e2e/encrypted.spec.ts`):

| Runner | Browser | EME ClearKey | Software fallback | Notes |
|--------|---------|-------------|-------------------|-------|
| `ubuntu-latest` | chromium | Yes | — | Native EME path |
| `ubuntu-latest` | firefox | Yes | — | Native EME path |
| `ubuntu-latest` | webkit | No | Yes | EME absent; Layer 1 detects, uses `softwareDecrypt.ts` |
| `macos-latest` | webkit | Probe: yes, actual: no | Yes | EME exists but silently fails; Layer 2 detects |
| `macos-latest` | firefox | Yes | — | Native EME path |
| `windows-latest` | edge | Yes | — | Native EME path |
